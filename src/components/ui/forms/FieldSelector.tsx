...
/**
 * Selector especializado en "elegir un campo" (custom + nativos), agrupado por sección --
 * "Datos del Lead"/"Creación"/"Modificación" para los nativos, la sección real (`lead_field_section`)
 * para los custom (ver `leadFieldUtils.ts`). Es el mismo componente usado en toda la app donde se
 * elige un campo (condiciones/acciones de Automatizaciones, filtros de la lista de leads,
 * condiciones de enrutamiento) para que un cambio de estilo/comportamiento se haga en un solo lugar.
 *
 * "Hereda" de `Selector` el toggle `searchable` -- ver ese componente para la diferencia entre modo
 * Autocomplete (con buscador) y modo Select simple.
 */
