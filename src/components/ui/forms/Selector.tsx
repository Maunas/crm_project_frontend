...
/**
 * Selector genérico value/onChange -- no depende de react-hook-form (para eso, envolver con un
 * `Controller` propio, como hace `ControlledAutocomplete` con `Autocomplete`). `searchable` decide
 * si se renderiza como un `Autocomplete` (con input de texto para filtrar) o como un `<Select>`
 * simple, sin duplicar la lógica de agrupamiento/opciones en cada lugar.
 *
 * Pensado como base para selectores especializados (ej. `FieldSelector`, que agrega el
 * agrupamiento por sección de campos nativos/custom) -- "heredan" el toggle `searchable` y el
 * resto de estas props sin reimplementarlas.
 */
