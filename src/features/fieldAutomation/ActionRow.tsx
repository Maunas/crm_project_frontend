...
export const ActionRow = ({ control, register, onDelete, isOnly, index, fields, nativeOptions, readOnly = false, onUpdate }: ActionRowProps) => {
  ...
  const allowedTargetFields = useMemo(() => {
    ...
    return fields.filter(f => {
      ...
      // Creador/Modificación): se pueden leer en condiciones y usar como origen de "Copiar de
      // otro campo", pero no tiene sentido "setearlos" a mano -- son hechos de auditoría.
      // Mismo criterio que `WRITABLE_NATIVE_FIELD_IDS` del backend (native_lead_fields.py).
