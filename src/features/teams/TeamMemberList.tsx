...
export const TeamMemberList = ({ team }: TeamMemberListProps) => {
    ...
    const fetchMembers = useCallback((fetchPage: number, pageSize: number, teamId: number) => {
        return getTeamMembers({ only_active: false, detailed: true, page: fetchPage, page_size: pageSize, team_id: teamId })
            .then(setMembers)
            ...
            <LoadingScreenWrapper loading={loading}>
                {members && members.items?.length > 0 ?
                    ...
                    : <Stack spacing={2} sx={{ justifyContent: "center", alignItems: "center" }}>
                        <Typography variant="h4">Este equipo todavía no tiene miembros...</Typography>
