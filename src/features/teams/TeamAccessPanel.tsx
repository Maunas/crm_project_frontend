...
const WorkspaceAccessSection = ({ team }: TeamAccessPanelProps) => {
    ...
    // a.workspace es el objeto anidado con el uuid real -- antes se comparaba a.workspace_id
    // (id interno) contra w.id (uuid) y nunca matcheaba, por lo que este filtro nunca excluía nada.
    const availableWorkspaces = useMemo(() =>
        workspaces.filter(w => !access.some(a => a.workspace?.id === w.id))
        , [workspaces, access])

    // workspace_id acá es el uuid de Workspace (retornado por ControlledAutocomplete vía
    // returnField="id"), no el id interno viejo.
...
const CampaignAccessSection = ({ team }: TeamAccessPanelProps) => {
    ...
    // a.campaign es el objeto anidado con el uuid real -- mismo bug que en
    // WorkspaceAccessSection (a.campaign_id vs c.id nunca matcheaba).
    const availableCampaigns = useMemo(() =>
        campaigns.filter(c => !access.some(a => a.campaign?.id === c.id))
        , [campaigns, access])

    // campaign_id acá es el uuid de Campaign (retornado por ControlledAutocomplete vía
    // returnField="id"), no el id interno viejo.
