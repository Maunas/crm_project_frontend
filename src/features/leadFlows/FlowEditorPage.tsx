...
export const LeadFlowEditor = () => {
  const { id } = useParams();
  // id es el public_uuid del flujo (Fase 3, ver backend/AGENTS.md §18) -- antes se usaba
  // parseInt(id), que rompía tanto la carga del flujo a editar como la URL resultante.
  ...
  const handleSave = async (flowName: string, flowDescription: string, states: FlowEditorState[], transitions: FlowEditorTransition[]) => {
    ...
    // etapas existentes viajan con su tempId real (public_uuid) tal cual -- antes se detectaba
    ...
    const statesPayload = states.map(s => {
      ...
      if (s.isNew) {
        ...
        backendId = s.tempId; // public_uuid real del estado existente
      }
    ...
    try {
      ...
      // Si era una creación, actualizamos el ID (public_uuid) y la URL sin recargar
