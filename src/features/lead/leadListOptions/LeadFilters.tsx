...
interface FormFilter {
    field_id?: number
    operator?: string
    value?: string | number | boolean   // STRING y tipos restantes; BOOL usa 'true'/'false'
    ...
    const onSubmit = async (data: LeadListFilters) => {
        ...
        for (const item of data.filters) {
            ...
            if (typeCode === 'STRING') {
                ...
                let newValue: string | number | boolean | undefined = item.value
                        ...
                        <Tooltip title="Aplicar filtros">
                            <span>
                                <IconButton size="small" type="submit" disabled={loading} color="primary"
                                    sx={{ bgcolor: 'primary.main', color: 'primary.contrastText', borderRadius: 1,
...
export const LeadFiltersItem = memo(({
    ...
    const collapsedSuffix = useMemo(() => {
        ...
        if (isNumeric) {
            const f = valueNumFrom != null && !isNaN(valueNumFrom) ? valueNumFrom : null
            const t = valueNumTo   != null && !isNaN(valueNumTo)   ? valueNumTo   : null
    ...
    if (compact) {
        return (
            <Box sx={{
                ...
                {/* Cabecera */}
                <Stack direction="row" alignItems="center" sx={{ px: 0.5, py: 0.25, minHeight: 30 }}>
                    {isFilled ? (
                    ...
                    {!isExpanded && isFilled ? (
                        ...
                        <Stack direction="row" spacing={0.5} alignItems="center"
                            sx={{ flex: 1, minWidth: 0, overflow: 'hidden', cursor: 'pointer' }}
...
const LeadFormFieldType = memo(({ register, control, name, leadField, errorMessage }: LeadFormFieldTypeProps) => {
    ...
    switch (typeCode) {
        ...
        case "FILE":
            return <LeadFormFile register={register} name={name} required={required} size="small" label="Valor" errorMessage={errorMessage} showAdornment />
        case "SELECTOR":
