...
import { Chip, Grid, List, ListItemText, Stack, Typography } from '@mui/material'
...
export const RoutingPolicyList = () => {
    ...
    const fetchPolicies = useCallback((fetchPage: number, pageSize: number, campaignId: number | null, teamId: number | null) => {
        ...
        if (teamId) {
            return getRoutingPolicies({ only_active: false, detailed: true, page_size: 0, ...(campaignId ? { campaign_id: campaignId } : {}) })
                .then(res => {
        ...
        <ContainerWithSidebar isSidebarOpen={Boolean(sidebarMode)} closeSidebar={closeSidebar} sidebarComponent={
            ...
            <Stack spacing={2}>
                ...
                <Stack direction="row" spacing={2} useFlexGap sx={{ alignItems: "center", flexWrap: "wrap" }}>
                    ...
                    {teamFilterId &&
                        <Chip label={`Equipo: ${teamName ?? teamFilterId}`} onDelete={clearTeamFilter} color="primary" />
                    }
                </Stack>
                    ...
                    <Stack spacing={2}>
                        {
                            policies && policies.items?.length > 0 ?
                                ...
                                : <Stack spacing={2} sx={{ justifyContent: "center", alignItems: "center" }}>
                                    <Typography variant="h4">No se han encontrado políticas de enrutamiento...</Typography>
